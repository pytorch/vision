...
def make_image_loaders(
    *,
    ...
    dtypes=(torch.float32, torch.float16, torch.float64, torch.uint8),
    constant_alpha=True,
):
...
def make_bounding_box_loaders(
    *,
    ...
    spatial_size="random",
    dtypes=(torch.float32, torch.float16, torch.float64, torch.int64),
):
...
def make_video_loaders(
    *,
    ...
    extra_dims=DEFAULT_EXTRA_DIMS,
    dtypes=(torch.uint8, torch.float32, torch.float16, torch.float64),
):
