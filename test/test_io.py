...
def temp_video(request):
    ...
    fps = request.param['fps']
    lossless = request.param.get('lossless', False)
    video_codec = request.param.get('video_codec', None)
    options = request.param.get('options', None)
...
class TestVideo:
    ...
    @pytest.mark.parametrize(
        'temp_video',
        [dict(num_frames=10, height=300, width=300, fps=5, lossless=True)],
        indirect=True
    )
    def test_write_read_video(self, temp_video):
        f_name, data = temp_video
    ...
    @pytest.mark.parametrize(
        'temp_video',
        [dict(num_frames=20, height=30, width=30, fps=10, lossless=True)],
        indirect=True
    )
    def test_write_read_video2(self, temp_video, dummy):
        # Same as above but passing different parameters to the temp_video fixture
        # The fixture should only be called **once** for a given set of parameters.
        f_name, data = temp_video
        lv, _, info = io.read_video(f_name)
        assert_equal(data, lv)
