        ...
        try:
            with self.create_dataset(config) as (dataset, info):
                for target_keys in [None, "all"]:
                    ...
                    assert tree_any(
                        lambda item: isinstance(item, (vision_tensors.VisionTensor, PIL.Image.Image)), wrapped_sample
                    )
...
def check_transforms_v2_wrapper_spawn(dataset):
    ...
    for wrapped_sample in dataloader:
        assert tree_any(
            lambda item: isinstance(item, (vision_tensors.Image, vision_tensors.Video, PIL.Image.Image)), wrapped_sample
        )
