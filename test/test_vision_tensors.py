...
@pytest.mark.parametrize(
    "format", ["XYXY", "CXCYWH", vision_tensors.BoundingBoxFormat.XYXY, vision_tensors.BoundingBoxFormat.XYWH]
)
def test_bbox_instance(data, format):
    bboxes = vision_tensors.BoundingBoxes(data, format=format, canvas_size=(32, 32))
    assert isinstance(bboxes, torch.Tensor)
    assert bboxes.ndim == 2 and bboxes.shape[1] == 4
    if isinstance(format, str):
        format = vision_tensors.BoundingBoxFormat[(format.upper())]
...
def test_bbox_dim_error():
    ...
    with pytest.raises(ValueError, match="Expected a 1D or 2D tensor, got 3D"):
        vision_tensors.BoundingBoxes(data_3d, format="XYXY", canvas_size=(32, 32))
...
def test_new_requires_grad(data, input_requires_grad, expected_requires_grad):
    vision_tensor = vision_tensors.Image(data, requires_grad=input_requires_grad)
    assert vision_tensor.requires_grad is expected_requires_grad
...
@pytest.mark.parametrize("make_input", [make_image, make_bounding_boxes, make_segmentation_mask, make_video])
@pytest.mark.parametrize("return_type", ["Tensor", "vision_tensor"])
def test_to_vision_tensor_reference(make_input, return_type):
    tensor = torch.rand((3, 16, 16), dtype=torch.float64)
    dp = make_input()

    with vision_tensors.set_return_type(return_type):
...
def test_force_subclass_with_metadata(return_type):
    # Sanity checks for the ops in _FORCE_TORCHFUNCTION_SUBCLASS and vision_tensors with metadata
    # Largely the same as above, we additionally check that the metadata is preserved
    format, canvas_size = "XYXY", (32, 32)
    bbox = vision_tensors.BoundingBoxes([[0, 0, 5, 5], [2, 2, 7, 7]], format=format, canvas_size=canvas_size)

    vision_tensors.set_return_type(return_type)
    bbox = bbox.clone()
    if return_type == "vision_tensor":
...
def test_wrap(make_input):
    ...
    dp_new = vision_tensors.wrap(output, like=dp)
...
def test_usual_operations(make_input, return_type, op):
    ...
    with vision_tensors.set_return_type(return_type):
        out = op(dp)
    assert type(out) is (type(dp) if return_type == "vision_tensor" else torch.Tensor)
    if isinstance(dp, vision_tensors.BoundingBoxes) and return_type == "vision_tensor":
...
def test_set_return_type():
    ...
    with vision_tensors.set_return_type("vision_tensor"):
        assert type(img + 3) is vision_tensors.Image
    assert type(img + 3) is torch.Tensor

    vision_tensors.set_return_type("vision_tensor")
    assert type(img + 3) is vision_tensors.Image

    with vision_tensors.set_return_type("tensor"):
        assert type(img + 3) is torch.Tensor
        with vision_tensors.set_return_type("vision_tensor"):
            assert type(img + 3) is vision_tensors.Image
            vision_tensors.set_return_type("tensor")
    ...
    # regardless of whether the "global" vision_tensors.set_return_type() was called within the context manager.
    assert type(img + 3) is vision_tensors.Image
