...
def make_bounding_box(*, format, image_size=(32, 32), extra_dims=(), dtype=torch.int64):
    ...
    if format == features.BoundingBoxFormat.XYXY:
        ...
        w = randint_with_tensor_bounds(1, torch.minimum(cx, width - cx))
        h = randint_with_tensor_bounds(1, torch.minimum(cy, width - cy))
        parts = (cx, cy, w, h)
...
class SampleInput:
    ...
    def __str__(self):
        def format_value(value):
            ...
            return f"{type(value).__name__}[{tuple(value.shape)}, {str(value.dtype).replace('torch.', '')}]"

        return "-".join(
            [
                *[format_value(arg) for arg in self.args],
                *[f"{param}={format_value(kwarg)}" for param, kwarg in self.kwargs.items()],
            ]
        )
...
class KernelInfos(UserList):
    def append_from_sample_inputs_fn(self, sample_inputs_fn):
        parts = sample_inputs_fn.__name__.split("sample_inputs_", 1)
        if len(parts) != 2:
            raise ValueError()

        self.data.append(KernelInfo(parts[1], sample_inputs_fn=sample_inputs_fn))

        return sample_inputs_fn


KERNEL_INFOS = KernelInfos()


@KERNEL_INFOS.append_from_sample_inputs_fn
def sample_inputs_horizontal_flip_image():
...
@KERNEL_INFOS.append_from_sample_inputs_fn
def sample_inputs_resize_image():
