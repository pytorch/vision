...
from torchvision.prototype import transforms, vision_tensors
...
from torchvision.vision_tensors import BoundingBoxes, BoundingBoxFormat, Image, Mask, Video
...
class TestSimpleCopyPaste:
    ...
    def test__extract_image_targets_assertion(self, mocker):
        ...
        flat_sample = [
            ...
            # labels, bboxes, masks
            mocker.MagicMock(spec=vision_tensors.Label),
    ...
    @pytest.mark.parametrize("label_type", [vision_tensors.Label, vision_tensors.OneHotLabel])
...
class TestLabelToOneHot:
    def test__transform(self):
        ...
        labels = vision_tensors.Label(torch.tensor([0, 1, 2, 1]), categories=categories)
...
def test_fixed_sized_crop_against_detection_reference():
    def make_vision_tensors():
        ...
        vision_tensor_image = make_image(size=size, color_space="RGB")
        target = {
            "boxes": make_bounding_boxes(canvas_size=size, format="XYXY", batch_dims=(num_objects,), dtype=torch.float),
            "labels": make_label(extra_dims=(num_objects,), categories=80),
            "masks": make_detection_mask(size=size, num_objects=num_objects, dtype=torch.long),
