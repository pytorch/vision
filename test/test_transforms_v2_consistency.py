...
from torchvision import transforms as legacy_transforms, vision_tensors
...
def check_call_consistency(
    ...
    for image in images:
        ...
        try:
            ...
            raise AssertionError(
                f"Transforming a image vision_tensor with shape {image_repr} failed in the prototype transform with "
                f"the error above. This means there is a consistency bug either in `_get_params` or in the "
                f"`vision_tensors.Image` path in `_transform`."
            ) from exc

        assert_close(
            output_prototype_image,
            output_prototype_tensor,
            msg=lambda msg: f"Output for vision_tensor and tensor images is not equal: \n\n{msg}",
            **closeness_kwargs,
        )
...
class TestAATransforms:
    @pytest.mark.parametrize(
        ...
        [
            ...
            vision_tensors.Image(torch.randint(0, 256, size=(1, 3, 256, 256), dtype=torch.uint8)),
        ],
    )
    @pytest.mark.parametrize(
        "interpolation",
...
class TestRefDetTransforms:
    def make_vision_tensors(self, with_mask=True):
        size = (600, 800)
        num_objects = 22
        ...
        vision_tensor_image = make_image(size=size, color_space="RGB", dtype=torch.float32)
        target = {
            "boxes": make_bounding_boxes(canvas_size=size, format="XYXY", batch_dims=(num_objects,), dtype=torch.float),
            "labels": make_label(extra_dims=(num_objects,), categories=80),
        }
        ...
        yield (vision_tensor_image, target)
    ...
    def test_transform(self, t_ref, t, data_kwargs):
        for dp in self.make_vision_tensors(**data_kwargs):
...
class TestRefSegTransforms:
    def make_vision_tensors(self, supports_pil=True, image_dtype=torch.uint8):
        size = (256, 460)
        num_categories = 21
        ...
        for conv_fn in conv_fns:
            vision_tensor_image = make_image(size=size, color_space="RGB", dtype=image_dtype)
            vision_tensor_mask = make_segmentation_mask(size=size, num_categories=num_categories, dtype=torch.uint8)

            dp = (conv_fn(vision_tensor_image), vision_tensor_mask)
            dp_ref = (
                to_pil_image(vision_tensor_image) if supports_pil else vision_tensor_image.as_subclass(torch.Tensor),
                to_pil_image(vision_tensor_mask),
            )
    ...
    def check(self, t, t_ref, data_kwargs=None):
        for dp, dp_ref in self.make_vision_tensors(**data_kwargs or dict()):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                v2_transforms.Compose(
                    [
                        PadIfSmaller(size=480, fill={vision_tensors.Mask: 255, "others": 0}),
