...
def auto_augment_adapter(transform, input, device):
    ...
    for key, value in input.items():
        if isinstance(value, (vision_tensors.BoundingBoxes, vision_tensors.Mask)):
            # AA transforms don't support bounding boxes or masks
            continue
        elif check_type(value, (vision_tensors.Image, vision_tensors.Video, is_pure_tensor, PIL.Image.Image)):
...
def normalize_adapter(transform, input, device):
    ...
    for key, value in input.items():
        if isinstance(value, PIL.Image.Image):
            ...
            continue
        elif check_type(value, (vision_tensors.Image, vision_tensors.Video, is_pure_tensor)):
...
class TestSmoke:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (transforms.ConvertBoundingBoxFormat(vision_tensors.BoundingBoxFormat.CXCYWH), None),
    ...
    def test_common(self, transform, adapter, container_type, image_or_video, de_serialize, device):
        ...
        input = dict(
            ...
            image_vision_tensor=make_image(size=canvas_size),
            video_vision_tensor=make_video(size=canvas_size),
            image_pil=next(make_pil_images(sizes=[canvas_size], color_spaces=["RGB"])),
            bounding_boxes_xyxy=make_bounding_boxes(
                format=vision_tensors.BoundingBoxFormat.XYXY, canvas_size=canvas_size, batch_dims=(3,)
            ),
            bounding_boxes_xywh=make_bounding_boxes(
                format=vision_tensors.BoundingBoxFormat.XYWH, canvas_size=canvas_size, batch_dims=(4,)
            ),
            bounding_boxes_cxcywh=make_bounding_boxes(
                format=vision_tensors.BoundingBoxFormat.CXCYWH, canvas_size=canvas_size, batch_dims=(5,)
            ),
            bounding_boxes_degenerate_xyxy=vision_tensors.BoundingBoxes(
                [
            ...
            bounding_boxes_degenerate_xywh=vision_tensors.BoundingBoxes(
                [
                    [0, 0, 0, 0],  # no height or width
                    [0, 0, 0, 1],  # no height
                    [0, 0, 1, 0],  # no width
                ...
                format=vision_tensors.BoundingBoxFormat.XYWH,
                canvas_size=canvas_size,
            ),
            bounding_boxes_degenerate_cxcywh=vision_tensors.BoundingBoxes(
                [
                    [0, 0, 0, 0],  # no height or width
                    [0, 0, 0, 1],  # no height
                    [0, 0, 1, 0],  # no width
                ...
                format=vision_tensors.BoundingBoxFormat.CXCYWH,
                canvas_size=canvas_size,
            ),
        ...
        for output_item, input_item, should_be_transformed in zip(
            ...
            if isinstance(input_item, vision_tensors.BoundingBoxes) and not isinstance(
        ...
        for format in list(vision_tensors.BoundingBoxFormat):
            sample = dict(
                boxes=vision_tensors.BoundingBoxes([[0, 0, 0, 0]], format=format, canvas_size=(224, 244)),
...
class TestTransform:
    @pytest.mark.parametrize(
        "inpt_type",
        [torch.Tensor, PIL.Image.Image, vision_tensors.Image, np.ndarray, vision_tensors.BoundingBoxes, str, int],
    )
...
class TestToImage:
    ...
    def test__transform(self, inpt_type, mocker):
        ...
        transform(inpt)
        if inpt_type in (vision_tensors.BoundingBoxes, vision_tensors.Image, str, int):
...
class TestToPILImage:
    ...
    def test__transform(self, inpt_type, mocker):
        ...
        transform(inpt)
        if inpt_type in (PIL.Image.Image, vision_tensors.BoundingBoxes, str, int):
            assert fn.call_count == 0
        else:
...
class TestToTensor:
    ...
    def test__transform(self, inpt_type, mocker):
        ...
        transform(inpt)
        if inpt_type in (vision_tensors.Image, torch.Tensor, vision_tensors.BoundingBoxes, str, int):
            assert fn.call_count == 0
        else:
            fn.assert_called_once_with(inpt)
...
class TestRandomIoUCrop:
    ...
    def test__transform_empty_params(self, mocker):
        ...
        image = vision_tensors.Image(torch.rand(1, 3, 4, 4))
        bboxes = vision_tensors.BoundingBoxes(torch.tensor([[1, 1, 2, 2]]), format="XYXY", canvas_size=(4, 4))
    ...
    def test__transform(self, mocker):
        ...
        assert isinstance(output_bboxes, vision_tensors.BoundingBoxes)
        assert (output_bboxes[~is_within_crop_area] == 0).all()

        output_masks = output[2]
        assert isinstance(output_masks, vision_tensors.Mask)
...
class TestLinearTransformation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            vision_tensors.Image(122 * torch.ones(1, 3, 8, 8)),
...
class TestUniformTemporalSubsample:
    @pytest.mark.parametrize(
        ...
        [
            ...
            vision_tensors.Video(torch.zeros(1, 10, 3, 8, 8)),
        ],
    )
    def test__transform(self, inpt):
        num_samples = 5
...
def test_antialias_warning():
    ...
    with pytest.warns(UserWarning, match=match):
        F.resized_crop(vision_tensors.Image(tensor_img), 0, 0, 10, 10, (20, 20))

    with pytest.warns(UserWarning, match=match):
        F.resize(vision_tensors.Video(tensor_video), (20, 20))
    with pytest.warns(UserWarning, match=match):
        F.resized_crop(vision_tensors.Video(tensor_video), 0, 0, 10, 10, (20, 20))
    ...
    with warnings.catch_warnings():
        ...
        F.resized_crop(vision_tensors.Image(tensor_img), 0, 0, 10, 10, (20, 20), antialias=True)
        F.resized_crop(vision_tensors.Video(tensor_video), 0, 0, 10, 10, (20, 20), antialias=True)


@pytest.mark.parametrize("image_type", (PIL.Image, torch.Tensor, vision_tensors.Image))
...
def test_classif_preset(image_type, label_type, dataset_return_type, to_tensor):
    ...
    image = vision_tensors.Image(torch.randint(0, 256, size=(1, 3, 250, 250), dtype=torch.uint8))
...
def test_detection_preset(image_type, data_augmentation, to_tensor, sanitize):
    ...
    if data_augmentation == "hflip":
        ...
        t = [
            ...
            #     size=(1024, 1024), fill=defaultdict(lambda: (123.0, 117.0, 104.0), {vision_tensors.Mask: 0})
            # ),
        ...
        t = [
            ...
            transforms.RandomZoomOut(fill={"others": (123.0, 117.0, 104.0), vision_tensors.Mask: 0}, p=1),
    ...
    image = vision_tensors.Image(torch.randint(0, 256, size=(1, 3, H, W), dtype=torch.uint8))
    if image_type is PIL.Image:
        image = to_pil_image(image[0])
    elif image_type is torch.Tensor:
        image = image.as_subclass(torch.Tensor)
    ...
    boxes = vision_tensors.BoundingBoxes(boxes, format="XYXY", canvas_size=(H, W))

    masks = vision_tensors.Mask(torch.randint(0, 2, size=(num_boxes, H, W), dtype=torch.uint8))
    ...
    if isinstance(to_tensor, transforms.ToTensor) and image_type is not vision_tensors.Image:
        assert is_pure_tensor(out["image"])
    else:
        assert isinstance(out["image"], vision_tensors.Image)
...
def test_sanitize_bounding_boxes(min_size, labels_getter, sample_type):
    ...
    boxes = vision_tensors.BoundingBoxes(
        boxes,
        format=vision_tensors.BoundingBoxFormat.XYXY,
        canvas_size=(H, W),
    )

    masks = vision_tensors.Mask(torch.randint(0, 2, size=(boxes.shape[0], H, W)))
    ...
    assert isinstance(out_boxes, vision_tensors.BoundingBoxes)
    assert isinstance(out_masks, vision_tensors.Mask)
...
def test_sanitize_bounding_boxes_no_label():
    ...
    assert isinstance(out_img, vision_tensors.Image)
    assert isinstance(out_boxes, vision_tensors.BoundingBoxes)


def test_sanitize_bounding_boxes_errors():

    good_bbox = vision_tensors.BoundingBoxes(
        [[0, 0, 10, 10]],
        format=vision_tensors.BoundingBoxFormat.XYXY,
        canvas_size=(20, 20),
    )
