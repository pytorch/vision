...
class TestSmoke:
    ...
    def test_common(self, transform, adapter, container_type, image_or_video, device):
        ...
        input = dict(
            ...
            bounding_box_degenerate_xyxy=datapoints.BBoxes(
                [
            ...
            bounding_box_degenerate_xywh=datapoints.BBoxes(
                [
                    [0, 0, 0, 0],  # no height or width
                    [0, 0, 0, 1],  # no height
                    [0, 0, 1, 0],  # no width
            ...
            bounding_box_degenerate_cxcywh=datapoints.BBoxes(
                [
                    [0, 0, 0, 0],  # no height or width
                    [0, 0, 0, 1],  # no height
                    [0, 0, 1, 0],  # no width
        ...
        for output_item, input_item, should_be_transformed in zip(
            ...
            if isinstance(input_item, datapoints.BBoxes) and not isinstance(
                transform, transforms.ConvertBBoxFormat
            ):
