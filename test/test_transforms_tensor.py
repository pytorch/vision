...
class Tester(unittest.TestCase):
    ...
    def test_class_interface(self):
        ...
        for config in configs:  # One parameter at the time
            f = T.ColorJitter(**config)
            scripted_fn = torch.jit.script(f)
            scripted_fn(tensor)

        f = T.ColorJitter(brightness=0.1, contrast=0.2, saturation=0.5, hue=0.5)  # All 4 parameters together
        scripted_fn = torch.jit.script(f)
        scripted_fn(tensor)
