...
class ROIPoolTester(unittest.TestCase):
                ...
                if roi[0] == n:
                    ...
                    for j in range(0, pool_h):
                        for i in range(0, pool_w):
                            y[r, :, j, i] = torch.max(y[r, :, j, i],
                                                      torch.max(roi_x[:, :,
                                                                      int(np.floor(j * bin_h)):int(np.ceil((j + 1) * bin_h)),
                                                                      int(np.floor(i * bin_w)):int(np.ceil((i + 1) * bin_w))])
                                                      )
        return y
    ...
    def test_roi_pool_gradient_cpu(self):
        ...
        layer = layers.ROIPool((5, 5), 1).to(dtype=self.dtype, device=device)
    ...
    def test_roi_pool_gradcheck_cpu(self):
        ...
        m = layers.ROIPool((5, 5), 1).to(dtype=self.dtype, device=device)
...
class NMSTester(unittest.TestCase):
    ...
    def test_nms_cuda(self):
        ...
        r_cuda = layers.nms(x.cuda().half(), scores.cuda().half(), 0.3)
        assert torch.allclose(r_cpu, r_cuda.cpu()), 'NMS incompatible between CPU and CUDA'
