...
class DispatcherInfo(InfoBase):
    ...
    def __init__(
        ...
        *,
        ...
        kernel_infos = {}
        for vision_tensor_type, kernel in self.kernels.items():
            kernel_info = self._KERNEL_INFO_MAP.get(kernel)
            if not kernel_info:
                raise pytest.UsageError(
                    f"Can't register {kernel.__name__} for type {vision_tensor_type} since there is no `KernelInfo` for it. "
                    f"Please add a `KernelInfo` for it in `transforms_v2_kernel_infos.py`."
                )
            kernel_infos[vision_tensor_type] = kernel_info
        self.kernel_infos = kernel_infos

    def sample_inputs(self, *vision_tensor_types, filter_metadata=True):
        for vision_tensor_type in vision_tensor_types or self.kernel_infos.keys():
            kernel_info = self.kernel_infos.get(vision_tensor_type)
            if not kernel_info:
            ...
            for args_kwargs in sample_inputs:
                if hasattr(vision_tensor_type, "__annotations__"):
                    for name in itertools.chain(
                        vision_tensor_type.__annotations__.keys(),
                        ...
                        (f"old_{name}" for name in vision_tensor_type.__annotations__.keys()),
                    ):
...
skip_dispatch_vision_tensor = TestMark(
    ("TestDispatchers", "test_dispatch_vision_tensor"),
    pytest.mark.skip(reason="Dispatcher doesn't support arbitrary vision_tensor dispatch."),
)
...
multi_crop_skips = [
    ...
    for test_name in ["test_pure_tensor_output_type", "test_pil_output_type", "test_vision_tensor_output_type"]
]
multi_crop_skips.append(skip_dispatch_vision_tensor)
...
DISPATCHER_INFOS = [
    DispatcherInfo(
        F.crop,
        kernels={
            vision_tensors.Image: F.crop_image,
            vision_tensors.Video: F.crop_video,
            vision_tensors.BoundingBoxes: F.crop_bounding_boxes,
            vision_tensors.Mask: F.crop_mask,
        },
    ...
    DispatcherInfo(
        F.resized_crop,
        kernels={
            vision_tensors.Image: F.resized_crop_image,
            vision_tensors.Video: F.resized_crop_video,
            vision_tensors.BoundingBoxes: F.resized_crop_bounding_boxes,
            vision_tensors.Mask: F.resized_crop_mask,
        },
    ...
    DispatcherInfo(
        F.pad,
        kernels={
            vision_tensors.Image: F.pad_image,
            vision_tensors.Video: F.pad_video,
            vision_tensors.BoundingBoxes: F.pad_bounding_boxes,
            vision_tensors.Mask: F.pad_mask,
        },
    ...
    DispatcherInfo(
        F.perspective,
        kernels={
            vision_tensors.Image: F.perspective_image,
            vision_tensors.Video: F.perspective_video,
            vision_tensors.BoundingBoxes: F.perspective_bounding_boxes,
    ...
    DispatcherInfo(
        F.elastic,
        kernels={
            vision_tensors.Image: F.elastic_image,
            vision_tensors.Video: F.elastic_video,
            vision_tensors.BoundingBoxes: F.elastic_bounding_boxes,
            vision_tensors.Mask: F.elastic_mask,
        },
    ...
    DispatcherInfo(
        F.center_crop,
        kernels={
            vision_tensors.Image: F.center_crop_image,
            vision_tensors.Video: F.center_crop_video,
            vision_tensors.BoundingBoxes: F.center_crop_bounding_boxes,
            vision_tensors.Mask: F.center_crop_mask,
        },
    ...
    DispatcherInfo(
        F.gaussian_blur,
        kernels={
            vision_tensors.Image: F.gaussian_blur_image,
            vision_tensors.Video: F.gaussian_blur_video,
        },
    ...
    DispatcherInfo(
        F.equalize,
        kernels={
            vision_tensors.Image: F.equalize_image,
            vision_tensors.Video: F.equalize_video,
        },
    ...
    DispatcherInfo(
        F.invert,
        kernels={
            vision_tensors.Image: F.invert_image,
            vision_tensors.Video: F.invert_video,
        },
    ...
    DispatcherInfo(
        F.posterize,
        kernels={
            vision_tensors.Image: F.posterize_image,
            vision_tensors.Video: F.posterize_video,
        },
    ...
    DispatcherInfo(
        F.solarize,
        kernels={
            vision_tensors.Image: F.solarize_image,
            vision_tensors.Video: F.solarize_video,
        },
    ...
    DispatcherInfo(
        F.autocontrast,
        kernels={
            vision_tensors.Image: F.autocontrast_image,
            vision_tensors.Video: F.autocontrast_video,
        },
    ...
    DispatcherInfo(
        F.adjust_sharpness,
        kernels={
            vision_tensors.Image: F.adjust_sharpness_image,
            vision_tensors.Video: F.adjust_sharpness_video,
        },
    ...
    DispatcherInfo(
        F.erase,
        kernels={
            vision_tensors.Image: F.erase_image,
            vision_tensors.Video: F.erase_video,
        },
        pil_kernel_info=PILKernelInfo(F._erase_image_pil),
        test_marks=[
            skip_dispatch_vision_tensor,
        ],
    ),
    DispatcherInfo(
        F.adjust_contrast,
        kernels={
            vision_tensors.Image: F.adjust_contrast_image,
            vision_tensors.Video: F.adjust_contrast_video,
        },
    ...
    DispatcherInfo(
        F.adjust_gamma,
        kernels={
            vision_tensors.Image: F.adjust_gamma_image,
            vision_tensors.Video: F.adjust_gamma_video,
        },
    ...
    DispatcherInfo(
        F.adjust_hue,
        kernels={
            vision_tensors.Image: F.adjust_hue_image,
            vision_tensors.Video: F.adjust_hue_video,
        },
    ...
    DispatcherInfo(
        F.adjust_saturation,
        kernels={
            vision_tensors.Image: F.adjust_saturation_image,
            vision_tensors.Video: F.adjust_saturation_video,
        },
    ...
    DispatcherInfo(
        F.five_crop,
        kernels={
            vision_tensors.Image: F.five_crop_image,
            vision_tensors.Video: F.five_crop_video,
        },
    ...
    DispatcherInfo(
        F.ten_crop,
        kernels={
            vision_tensors.Image: F.ten_crop_image,
            vision_tensors.Video: F.ten_crop_video,
        },
        test_marks=[
            xfail_jit_python_scalar_arg("size"),
            *multi_crop_skips,
    ...
    DispatcherInfo(
        F.normalize,
        kernels={
            vision_tensors.Image: F.normalize_image,
            vision_tensors.Video: F.normalize_video,
        },
        test_marks=[
    ...
    DispatcherInfo(
        ...
        kernels={
            vision_tensors.Video: F.uniform_temporal_subsample_video,
        },
        test_marks=[
            skip_dispatch_vision_tensor,
        ],
    ...
    DispatcherInfo(
        ...
        kernels={vision_tensors.BoundingBoxes: F.clamp_bounding_boxes},
        test_marks=[
            skip_dispatch_vision_tensor,
        ],
    ),
    DispatcherInfo(
        ...
        kernels={vision_tensors.BoundingBoxes: F.convert_bounding_box_format},
        test_marks=[
            skip_dispatch_vision_tensor,
        ],
    ),
