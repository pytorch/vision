...
class TestResize:
    ...
    def test_resize(self, height, width, osize, max_size):
        ...
        if height < width:
            ...
            assert result.size == (exp_w, exp_h)
        elif width < height:
    ...
    def test_resize_sequence_output(self, height, width, osize):
        img = Image.new("RGB", size=(width, height), color=127)
    ...
    def test_resize_antialias_error(self):
        osize = [37, 37]
...
class TestRandomX:
    ...
    def test_randomness(self, fn, trans, config, p):
    ...
    def test_random_rotation_error(self):
        # assert fill being either a Sequence or a Number
        with pytest.raises(TypeError):
    ...
    def test_randomperspective_fill(self, mode):
...
class TestCrop:
    ...
    def test_ten_crop(self, should_vflip, single_dim):
        to_pil_image = transforms.ToPILImage()
    ...
    @pytest.mark.parametrize('single_dim', [True, False])
    def test_five_crop(self, single_dim):
        to_pil_image = transforms.ToPILImage()
        h = random.randint(5, 25)
        w = random.randint(5, 25)
        crop_h = random.randint(1, h)
    ...
    def test_center_crop_2(self, odd_image_size, delta, delta_width, delta_height):
...
class TestNormalize:
    ...
    def test_normalize_different_dtype(self, dtype1, dtype2):
