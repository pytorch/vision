...
from torchvision import vision_tensors  # we'll describe this a bit later, bare with us
...
# (:class:`torchvision.vision_tensors.Mask`) for object segmentation or semantic
# segmentation, or videos (:class:`torchvision.vision_tensors.Video`), we could have
# passed them to the transforms in exactly the same way.
#
# By now you likely have a few questions: what are these vision_tensors, how do we
# use them, and what is the expected input/output of those transforms? We'll
# answer these in the next sections.

# %%
#
# .. _what_are_vision_tensors:
#
# What are VisionTensors?
# -----------------------
#
# VisionTensors are :class:`torch.Tensor` subclasses. The available vision_tensors are
# :class:`~torchvision.vision_tensors.Image`,
# :class:`~torchvision.vision_tensors.BoundingBoxes`,
# :class:`~torchvision.vision_tensors.Mask`, and
...
# or any ``torch.*`` operator will also work on a vision_tensor:

img_dp = vision_tensors.Image(torch.randint(0, 256, (3, 256, 256), dtype=torch.uint8))

print(f"{isinstance(img_dp, torch.Tensor) = }")
print(f"{img_dp.dtype = }, {img_dp.shape = }, {img_dp.sum() = }")

# %%
# These VisionTensor classes are at the core of the transforms: in order to
...
#
# You don't need to know much more about vision_tensors at this point, but advanced
...
#     :class:`~torchvision.vision_tensors.Image` class at all, and yet our images
...
#     or just ignored:
#
#     * If there is an :class:`~torchvision.vision_tensors.Image`,
#       :class:`~torchvision.vision_tensors.Video`,
#       or :class:`PIL.Image.Image` instance in the input, all other pure
#       tensors are passed-through.
#     * If there is no :class:`~torchvision.vision_tensors.Image` or
#       :class:`~torchvision.vision_tensors.Video` instance, only the first pure
...
# vision_tensors, so they don't return vision_tensors out of the box.
#
# An easy way to force those datasets to return vision_tensors and to make them
...
#    # Now the dataset returns vision_tensors!
#
# Using your own datasets
# ^^^^^^^^^^^^^^^^^^^^^^^
#
# If you have a custom dataset, then you'll need to convert your objects into
# the appropriate VisionTensor classes. Creating VisionTensor instances is very easy,
# refer to :ref:`vision_tensor_creation` for more details.
#
