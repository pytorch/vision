...
setup_conda_pytorch_constraint() {
  if [[ -z "$PYTORCH_VERSION" ]]; then
    ...
    export PYTORCH_VERSION="$(conda search --json 'pytorch[channel=pytorch-nightly]' | python -c "import os, sys, json, re; print(re.sub(r'\\+.*$', '', [x['version'] for x in json.load(sys.stdin)['pytorch'] if (x['platform'] == 'darwin' or os.environ['CU_VERSION'] in x['fn']) and 'py' + os.environ['PYTHON_VERSION'] in x['fn']][-1]))")"
  else
