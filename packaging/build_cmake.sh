...
if [[ $CU_VERSION == "cu116" ]]; then
    conda install -yq \pytorch=$PYTORCH_VERSION  -c nvidia cuda=11.6 atalman::pytorch-cuda=1.0=cuda11.6 $PYTORCH_MUTEX_CONSTRAINT $MKL_CONSTRAINT numpy -c nvidia -c "pytorch-${UPLOAD_CHANNEL}"
else
