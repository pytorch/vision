...
def patch_linux():
    ...
    for wheel in wheels:
        ...
        print('{0}'.format(osp.basename(wheel)))
        unzip_file(wheel, output_dir)

        print('Finding ELF dependencies...')

        output_library = osp.join(output_dir, 'torchvision')
        image_binary_path = osp.join(output_library, image_binary)

        ld_tree = lddtree(image_binary_path)
        tree_libs = ld_tree['libs']

        binary_queue = [(n, image_binary) for n in ld_tree['needed']]
        binary_paths = {image_binary: image_binary_path}
        binary_dependencies = {}
