...
# register_fake requires the C++ operator to already exist. Skip when the
# extension failed to load so `import torchvision` still succeeds; nms()/qnms()
# then raise via _assert_has_ops() with the cached load error.
if torchvision.extension._has_ops():
    ...
    @torch.library.register_fake("torchvision::qnms")
    def meta_qnms(dets, scores, iou_threshold):
        torch._check(dets.dim() == 2, lambda: f"boxes should be a 2d tensor, got {dets.dim()}D")
        torch._check(dets.size(1) == 4, lambda: f"boxes should have 4 elements in dimension 1, got {dets.size(1)}")
        torch._check(scores.dim() == 1, lambda: f"scores should be a 1d tensor, got {scores.dim()}")
        torch._check(
