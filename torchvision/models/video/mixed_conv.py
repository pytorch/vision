...
from ._utils import video_3d_conv, video_2d_conv
...
def mcX(model_depth, X=3, use_pool1=False, **kwargs):
    ...
    """
    assert X > 1 and X <= 5
    conv_makers = [video_3d_conv] * (X-2)
    while len(conv_makers) < 5:
        conv_makers.append(video_2d_conv)
...
def rmcX(model_depth, X=3, use_pool1=False, **kwargs):
    ...
    conv_makers = [video_2d_conv] * (X-2)
    while len(conv_makers) < 5:
        conv_makers.append(video_3d_conv)
