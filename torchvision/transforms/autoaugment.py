...
class PatchMix(torch.nn.Module):
    ...
    def __init__(
        self, num_classes: int, prob: float = 0.5, mix_num: int = 2, patch_size: int = 16, smoothing: float = 0.0
    ) -> None:
        super().__init__()
        self.prob = prob
    ...
    def _one_hot(self, target: Tensor, num_classes: int, on_value: float = 1.0, off_value: float = 0.0) -> Tensor:
