...
class RandomAffine(Transform):
    ...
    def make_params(self, flat_inputs: list[Any]) -> dict[str, Any]:
        ...
        if self.translate is not None:
            ...
            tx = int(round(float(torch.empty(1).uniform_(-max_dx, max_dx, generator=g).item())))
            ty = int(round(float(torch.empty(1).uniform_(-max_dy, max_dy, generator=g).item())))
        ...
        if self.shear is not None:
            shear_x = float(torch.empty(1).uniform_(self.shear[0], self.shear[1], generator=g).item())
            if len(self.shear) == 4:
                shear_y = float(torch.empty(1).uniform_(self.shear[2], self.shear[3], generator=g).item())
