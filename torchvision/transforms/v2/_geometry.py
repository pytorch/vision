...
class RandomHorizontalFlip(_RandomApplyTransform):
    ...
    If the input is a :class:`torch.Tensor` or a ``VisionTensor`` (e.g. :class:`~torchvision.vision_tensors.Image`,
    :class:`~torchvision.vision_tensors.Video`, :class:`~torchvision.vision_tensors.BoundingBoxes` etc.)
...
class FiveCrop(Transform):
    ...
    If the input is a :class:`torch.Tensor` or a :class:`~torchvision.vision_tensors.Image` or a
    :class:`~torchvision.vision_tensors.Video` it can have arbitrary number of leading batch dimensions.
    ...
    Example:
        ...
        ...     def forward(self, sample: Tuple[Tuple[Union[vision_tensors.Image, vision_tensors.Video], ...], int]):
        ...         images_or_videos, labels = sample
        ...         batch_size = len(images_or_videos)
        ...         image_or_video = images_or_videos[0]
        ...         images_or_videos = vision_tensors.wrap(torch.stack(images_or_videos), like=image_or_video)
        ...         labels = torch.full((batch_size,), label, device=images_or_videos.device)
        ...         return images_or_videos, labels
        ...
        >>> image = vision_tensors.Image(torch.rand(3, 256, 256))
        >>> label = 3
    ...
    def _call_kernel(self, functional: Callable, inpt: Any, *args: Any, **kwargs: Any) -> Any:
        if isinstance(inpt, (vision_tensors.BoundingBoxes, vision_tensors.Mask)):
            warnings.warn(
                f"{type(self).__name__}() is currently passing through inputs of type "
                f"vision_tensors.{type(inpt).__name__}. This will likely change in the future."
            )
    ...
    def _check_inputs(self, flat_inputs: List[Any]) -> None:
        if has_any(flat_inputs, vision_tensors.BoundingBoxes, vision_tensors.Mask):
...
class Pad(Transform):
    ...
    Args:
        ...
        fill (number or tuple or dict, optional): Pixel fill value used when the  ``padding_mode`` is constant.
            ...
            ``fill={vision_tensors.Image: 127, vision_tensors.Mask: 0}`` where ``Image`` will be filled with 127 and
...
class RandomIoUCrop(Transform):
    ...
    This transformation requires an image or video data and ``vision_tensors.BoundingBoxes`` in the input.
    ...
    def _check_inputs(self, flat_inputs: List[Any]) -> None:
        if not (
            has_all(flat_inputs, vision_tensors.BoundingBoxes)
            and has_any(flat_inputs, PIL.Image.Image, vision_tensors.Image, is_pure_tensor)
        ):
            raise TypeError(
    ...
    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:
        ...
        if isinstance(output, vision_tensors.BoundingBoxes):
