...
def horizontal_flip_keypoints(kp: tv_tensors.KeyPoints):
    return kp.sub_(kp.canvas_size[1]).neg_()
...
def _affine_keypoints_with_expand(
    ...
    # 1) Unlike bounding box (whose implmentation we stole) we're already a bunch of points.
    keypoints = torch.cat([keypoints, torch.ones(keypoints.shape[0], 1, device=device, dtype=dtype)], dim=-1)
...
@_register_kernel_internal(affine, tv_tensors.KeyPoints)
...
@_register_kernel_internal(pad, tv_tensors.KeyPoints, tv_tensors_wrapper=False)
def _pad(
...
def elastic_keypoints(kp: torch.Tensor, canvas_size: Tuple[int, int], displacement: torch.Tensor) -> torch.Tensor:
    ...
    return clamp_keypoints(transformed_points, canvas_size=canvas_size)
