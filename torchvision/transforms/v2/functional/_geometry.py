...
def horizontal_flip_bounding_box(
    bounding_box: torch.Tensor, format: datapoints.BBoxFormat, spatial_size: Tuple[int, int]
) -> torch.Tensor:
...
def _affine_bounding_box_with_expand(
    ...
    bounding_box = (
        convert_format_bounding_box(
            bounding_box, old_format=format, new_format=datapoints.BBoxFormat.XYXY, inplace=True
        )
    ).reshape(-1, 4)
    ...
    out_bboxes = clamp_bounding_box(out_bboxes, format=datapoints.BBoxFormat.XYXY, spatial_size=spatial_size)
...
def perspective_bounding_box(
    ...
    bounding_box = (
        convert_format_bounding_box(bounding_box, old_format=format, new_format=datapoints.BBoxFormat.XYXY)
    ).reshape(-1, 4)
