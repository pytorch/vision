...
class Query:
    ...
    def image_for_size_extraction(self) -> Image:
        def fn(sample: Any) -> Optional[Image]:
            if isinstance(sample, Image):
                return sample
            ...
            return Image(torch.empty(0, height, width))

        return self.query(fn)

    def image_for_size_and_channels_extraction(self) -> Image:
        def fn(sample: Any) -> Optional[Image]:
            if isinstance(sample, Image):
                return sample
            ...
            return Image(torch.empty(0, num_channels, height, width))
