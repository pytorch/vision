...
from torchvision.prototype import vision_tensors as proto_vision_tensors
...
class SimpleCopyPaste(Transform):
    ...
    def _copy_paste(
        self,
        image: Union[torch.Tensor, vision_tensors.Image],
        target: Dict[str, Any],
        paste_image: Union[torch.Tensor, vision_tensors.Image],
        ...
        paste_masks = vision_tensors.wrap(paste_target["masks"][random_selection], like=paste_target["masks"])
        paste_boxes = vision_tensors.wrap(paste_target["boxes"][random_selection], like=paste_target["boxes"])
        paste_labels = vision_tensors.wrap(paste_target["labels"][random_selection], like=paste_target["labels"])
        ...
        boxes = F.convert_bounding_box_format(
            xyxy_boxes, old_format=vision_tensors.BoundingBoxFormat.XYXY, new_format=bbox_format, inplace=True
        )
    ...
    ) -> Tuple[List[Union[torch.Tensor, vision_tensors.Image]], List[Dict[str, Any]]]:
        # fetch all images, bboxes, masks and labels from unstructured input
        # with List[image], List[BoundingBoxes], List[Mask], List[Label]
        images, bboxes, masks, labels = [], [], [], []
        for obj in flat_sample:
            if isinstance(obj, vision_tensors.Image) or is_pure_tensor(obj):
                images.append(obj)
            elif isinstance(obj, PIL.Image.Image):
                images.append(F.to_image(obj))
            elif isinstance(obj, vision_tensors.BoundingBoxes):
                bboxes.append(obj)
            elif isinstance(obj, vision_tensors.Mask):
                masks.append(obj)
            elif isinstance(obj, (proto_vision_tensors.Label, proto_vision_tensors.OneHotLabel)):
                labels.append(obj)
    ...
    def _insert_outputs(
        ...
        for i, obj in enumerate(flat_sample):
            if isinstance(obj, vision_tensors.Image):
                flat_sample[i] = vision_tensors.wrap(output_images[c0], like=obj)
                c0 += 1
            elif isinstance(obj, PIL.Image.Image):
                ...
                flat_sample[i] = vision_tensors.wrap(output_targets[c1]["boxes"], like=obj)
                c1 += 1
            elif isinstance(obj, vision_tensors.Mask):
                flat_sample[i] = vision_tensors.wrap(output_targets[c2]["masks"], like=obj)
                c2 += 1
            elif isinstance(obj, (proto_vision_tensors.Label, proto_vision_tensors.OneHotLabel)):
                flat_sample[i] = vision_tensors.wrap(output_targets[c3]["labels"], like=obj)
                c3 += 1
