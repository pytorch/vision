...
class PermuteDimensions(Transform):
    ...
    def __init__(self, dims: Union[Sequence[int], Dict[Type, Optional[Sequence[int]]]]) -> None:
        ...
        if torch.Tensor in dims and any(cls in dims for cls in [vision_tensors.Image, vision_tensors.Video]):
            warnings.warn(
                "Got `dims` values for `torch.Tensor` and either `vision_tensors.Image` or `vision_tensors.Video`. "
