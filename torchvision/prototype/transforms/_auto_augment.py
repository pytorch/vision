...
from ._utils import query_images
...
class AugMix(_AutoAugmentBase):
    _PARTIAL_AUGMENTATION_SPACE = {
        "ShearX": (lambda num_bins, image_size: torch.linspace(0.0, 0.3, num_bins), True),
        "ShearY": (lambda num_bins, image_size: torch.linspace(0.0, 0.3, num_bins), True),
        "TranslateX": (lambda num_bins, image_size: torch.linspace(0.0, image_size[0] / 3.0, num_bins), True),
        "TranslateY": (lambda num_bins, image_size: torch.linspace(0.0, image_size[1] / 3.0, num_bins), True),
        "Rotate": (lambda num_bins, image_size: torch.linspace(0.0, 30.0, num_bins), True),
        "Posterize": (
    ...
    def _apply_augmix(self, input: Any) -> Any:
        ...
        for i in range(self.mixture_width):
            ...
            for _ in range(depth):
                ...
                magnitudes = magnitudes_fn(self._PARAMETER_MAX, image_size)
                if magnitudes is not None:
