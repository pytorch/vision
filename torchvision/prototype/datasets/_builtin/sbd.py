...
from torchvision.prototype.datapoints import _Datapoint
...
class SBD(Dataset):
    ...
    def _prepare_sample(self, data: Tuple[Tuple[Any, Tuple[str, BinaryIO]], Tuple[str, BinaryIO]]) -> Dict[str, Any]:
        ...
        return dict(
            ...
            boundaries=_Datapoint(np.stack([raw_boundary.toarray() for raw_boundary in anns["Boundaries"].item()])),
            segmentation=_Datapoint(anns["Segmentation"].item()),
        )
