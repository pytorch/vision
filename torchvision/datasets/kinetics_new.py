...
class KineticsRandomDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        # FIXME: maybe remove
        self.alpha = alpha  # tolerance to avoid rounding errros with max seek time
    ...
    def __iter__(self):
        for i in range(self.epoch_size):
            ...
            print(f"video = {path}, duration = {duration}, max_seek = {max_seek}, start = {start}")
            if max_seek > 1:
            ...
            video = torch.stack(video_frames, 0) if video_frames else torch.zeros(16, 100, 100, 3, device=self.device)
