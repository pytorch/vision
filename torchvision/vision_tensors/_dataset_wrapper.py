...
from torchvision import datasets, vision_tensors
...
def wrap_dataset_for_transforms_v2(dataset, target_keys=None):
    ...
    Special cases:
        ...
        * :class:`~torchvision.datasets.CocoDetection`: Instead of returning the target as list of dicts, the wrapper
          ...
          ``"masks"`` and ``"labels"`` are added and wrap the data in the corresponding ``torchvision.vision_tensors``.
        ...
        * :class:`~torchvision.datasets.VOCDetection`: The key-value-pairs ``"boxes"`` and ``"labels"`` are added to
          the target and wrap the data in the corresponding ``torchvision.vision_tensors``. The original keys are
        ...
        * :class:`~torchvision.datasets.CelebA`: The target for ``target_type="bbox"`` is converted to the ``XYXY``
          coordinate format and wrapped into a :class:`~torchvision.vision_tensors.BoundingBoxes` vision_tensor.
        * :class:`~torchvision.datasets.Kitti`: Instead returning the target as list of dicts, the wrapper returns a
          ...
          in the corresponding ``torchvision.vision_tensors``. The original keys are preserved. If ``target_keys`` is
        ...
        * :class:`~torchvision.datasets.OxfordIIITPet`: The target for ``target_type="segmentation"`` is wrapped into a
          :class:`~torchvision.vision_tensors.Mask` vision_tensor.
        * :class:`~torchvision.datasets.Cityscapes`: The target for ``target_type="semantic"`` is wrapped into a
          :class:`~torchvision.vision_tensors.Mask` vision_tensor. The target for ``target_type="instance"`` is *replaced* by
          a dictionary with the key-value-pairs ``"masks"`` (as :class:`~torchvision.vision_tensors.Mask` vision_tensor) and
          ``"labels"``.
    ...
    Segmentation datasets
        ...
        segmentation mask into a :class:`~torchvision.vision_tensors.Mask` (second item).
    ...
    Video classification datasets
        ...
        :class:`~torchvision.vision_tensors.Video` while leaving the other items as is.
    ...
    # "WrappedImageNet" at runtime that doubly inherits from VisionDatasetVisionTensorWrapper (see below) as well as the
    ...
    wrapped_dataset_cls = type(
        f"Wrapped{type(dataset).__name__}", (VisionDatasetVisionTensorWrapper, type(dataset)), {}
    )
    # Since VisionDatasetVisionTensorWrapper comes before ImageNet in the MRO, calling the class hits
    # VisionDatasetVisionTensorWrapper.__init__ first. Since we are never doing super().__init__(...), the constructor of
...
class VisionDatasetVisionTensorWrapper:
    def __init__(self, dataset, target_keys):
        ...
        if not isinstance(dataset, datasets.VisionDataset):
            raise TypeError(
                ...
                "https://pytorch.org/vision/main/auto_examples/plot_vision_tensors.html#do-i-have-to-wrap-the-output-of-the-datasets-myself"
            )
...
def pil_image_to_mask(pil_image):
    return vision_tensors.Mask(pil_image)
...
def video_classification_wrapper_factory(dataset, target_keys):
    ...
    def wrapper(idx, sample):
        ...
        video = vision_tensors.Video(video)
...
def coco_dectection_wrapper_factory(dataset, target_keys):
    ...
    def wrapper(idx, sample):
        ...
        if "masks" in target_keys:
            target["masks"] = vision_tensors.Mask(
                torch.stack(
                    [
...
def voc_detection_wrapper_factory(dataset, target_keys):
    ...
    def wrapper(idx, sample):
        ...
        if "boxes" in target_keys:
            target["boxes"] = vision_tensors.BoundingBoxes(
                [
    ...
    def instance_segmentation_wrapper(mask):
        ...
        for id in data.unique():
            ...
            if label >= 1_000:
                label //= 1_000
            labels.append(label)
        return dict(masks=vision_tensors.Mask(torch.stack(masks)), labels=torch.stack(labels))
