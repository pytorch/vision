...
struct MediaFormat {
  ...
  bool operator<(const MediaFormat& x) const { return type < x.type; }
...
using DecoderOutCallback =
  std::function<void(DecoderOutputMessage&&)>;
...
class MediaDecoder {
 public:
   ...
   */
  virtual bool init(const DecoderParameters& params,
