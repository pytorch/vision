  ...
  switch (type) {
    ...
    case TYPE_VIDEO:
      return std::make_unique<VideoStream>(
          ...
          ctx, idx, convertPtsToWallTime, format.video, -loggingUuid);
    case TYPE_SUBTITLE:
...
void Decoder::logFunction(
  ...
  if (decoder != nullptr && decoder->enableLogLevel(level)) {
    ...
    *prefix = 1;
    av_log_format_line(
        avcl, level, cfmt, vl, buf, sizeof(buf) - 1, prefix);
...
bool Decoder::init(const DecoderParameters& params,
  ...
  if (params.startOffsetMs != 0) {
    av_seek_frame(inputCtx_, -1, params.startOffsetMs * AV_TIME_BASE / 1000,
