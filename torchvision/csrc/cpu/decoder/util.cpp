...
// fixed size types
template<typename T>
inline size_t getSize(const T& x) { return sizeof(x); }

template<typename T>
inline bool serializeItem(uint8_t* dest, size_t len, size_t& pos,
                          const T& src) {
...
template<typename T>
inline bool deserializeItem(const uint8_t* src, size_t len, size_t& pos,
                            T& dest) {
...
inline size_t getSize(const AVSubtitle& x) {
  ...
  return getSize(x.format) +
         getSize(x.start_display_time) +
         getSize(x.end_display_time) +
         getSize(x.pts) +
         rectBytes(x);
}
...
inline bool serializeItem(uint8_t* dest, size_t len, size_t& pos,
  ...
  auto rectSerialize = [](uint8_t* d, size_t l, size_t& p,
                          const AVSubtitleRect& x)-> size_t {
    switch (x.type) {
      case SUBTITLE_BITMAP:
...
inline bool deserializeItem(const uint8_t* src, size_t len, size_t& pos,
                            AVSubtitleRect& dest) {
  auto rectDeserialize = [](const uint8_t* y, size_t l, size_t& p,
                            AVSubtitleRect& x)-> bool {
    switch (x.type) {
      case SUBTITLE_BITMAP:
        for (int i = 0; i < x.nb_colors; ++i) {
