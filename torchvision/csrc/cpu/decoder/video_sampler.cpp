...
namespace {
int preparePlanes(const VideoFormat& fmt,
...
int transformImage(
    ...
    int lines[]) {
  int result;
  if ((result = preparePlanes(outFormat, out, planes, lines)) <
      0) {
    return result;
  }
...
bool VideoSampler::init(const SamplerParameters& params) {
  ...
  VLOG(1) << "Input format #" << loggingUuid_
          << ", width " << params.in.video.width
          << ", height " << params.in.video.height
          << ", format " << params.in.video.format
          << ", minDimension " << params.in.video.minDimension
          << ", cropImage " << params.in.video.cropImage;
  ...
  VLOG(1) << "Crop format #" << loggingUuid_ << ", width "
          << params.out.video.width
          << ", height " << params.out.video.height << ", format "
          << params.out.video.format << ", minDimension "
          << params.out.video.minDimension
          << ", cropImage " << params.out.video.cropImage;
  ...
  if (cropContext_) {
    ...
    if (params_.out.video.height < scaleFormat_.height) {
      ...
      for (size_t i = 0; i < 4 && scalePlanes[i] != nullptr; ++i) {
        scalePlanes[i] +=
          scaleLines[i] * (scaleFormat_.height - params_.out.video.height) / 2;
      }
    } else {
      // Source image is wider of destination image: cut sides
      for (size_t i = 0; i < 4 && scalePlanes[i] != nullptr; ++i) {
        scalePlanes[i] += scaleLines[i] *
        (scaleFormat_.width - params_.out.video.width) / 2 / scaleFormat_.width;
      }
    }
...
int VideoSampler::sample(const ByteStorage* in, ByteStorage* out) {
  ...
  if ((result = preparePlanes(params_.in.video,
                             in->data(),
                             inPlanes,
                             inLineSize)) < 0) {
    return result;
