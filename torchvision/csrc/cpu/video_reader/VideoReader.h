...
torch::List<torch::Tensor> readVideoFromMemory(
    ...
    // If only pts is needed and video/audio frames are not needed, set it to 1
    int64_t getPtsOnly,
    /*
    ...
    */
    ...
    // (denominator). For decoding localized video frames, need to supply them
    // which will be checked during decoding
    ...
    // (denominator). For decoding localized audio frames, need to supply them
    // which will be checked during decoding
