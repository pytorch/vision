...
Tensor decode_image(
  ...
  VISION_CHECK(data.device() == Device(kCPU), "Expected a CPU tensor");
  // Check that the input tensor dtype is uint8
  VISION_CHECK(data.scalar_type() == kByte, "Expected a torch.uint8 tensor");
  ...
  VISION_CHECK(data.numel() >= 3, err_msg);
  ...
  VISION_CHECK(data.numel() >= 4, err_msg);
  ...
  VISION_CHECK(data.numel() >= 6, err_msg);
