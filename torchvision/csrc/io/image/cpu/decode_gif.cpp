...
// `buf`. The data is read from the UserData field of the `gifFile` paramater which
...
torch::Tensor decode_gif(const torch::Tensor& encoded_bytes) {
    ...
    // Note:
    // - We're assuming and enforcing that all images have the same height and width
    // - We're ignoring the canvas width and height and assume shape is that of the first image
    // - We're ignoring the top and left offsets and enforce them to be 0.
    // This is for simplicity. All of those may be revisited in the future if need be.
    ...
    auto out = torch::empty({int64_t(num_images), 3, int64_t(out_h), int64_t(out_w)}, torch::kU8);
    ...
    for(int i = 0; i < num_images; i++) {
        ...
        TORCH_CHECK(colorMap != nullptr, "ColorMap is missing.");
        TORCH_CHECK(
