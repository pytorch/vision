...
struct MNASNetInvertedResidualImpl : torch::nn::Module
{
    ...
    MNASNetInvertedResidualImpl(int64_t input, int64_t output, int64_t kernel,
                                int64_t stride, double expansion_factor,
    ...
    {
        if (apply_residual) return layers->forward(x) + x;
...
int64_t round_to_multiple_of(int64_t val, int64_t divisor,
