...
void qroi_align_forward_kernel_impl(
  ...
  const float* rois = t_rois_cont.data_ptr<float>();
        ...
        for (int pw = 0; pw < pooled_width; pw++) {
          ...
          for (int iy = 0; iy < roi_bin_grid_h; iy++) {
            for (int ix = 0; ix < roi_bin_grid_w; ix++) {
              ...
              // Optimization: we use the raw values here and we'll
              // dequantize later
              output_val += pc.w1 * static_cast<float>(offset_input[pc.pos1]) +
                  pc.w2 * static_cast<float>(offset_input[pc.pos2]) +
                  pc.w3 * static_cast<float>(offset_input[pc.pos3]) +
                  pc.w4 * static_cast<float>(offset_input[pc.pos4]);
          ...
          constexpr int64_t qmax =
              static_cast<int64_t>(std::numeric_limits<T>::max());
          qval = std::max(qval, qmin);
          qval = std::min(qval, qmax);
          output[index] = static_cast<T>(qval);
        } // for pw
      } // for ph
    } // for c
  } // for n
...
at::Tensor qroi_align_forward_kernel(
  ...
  TORCH_CHECK(rois.scalar_type() == at::kFloat, "rois must be a float tensor");
...
TORCH_LIBRARY_FRAGMENT(torchvision, m) {
  m.def(TORCH_SELECTIVE_SCHEMA(
      "torchvision::qroi_align(Tensor input, Tensor rois, float input_scale, int input_zero_point, float spatial_scale, SymInt pooled_height, SymInt pooled_width, int sampling_ratio, bool aligned) -> Tensor"));
}
