...
void launch_ps_roi_align_forward_kernel_impl(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    int nthreads,
...
template <typename scalar_t>
void launch_ps_roi_align_backward_kernel_impl(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    int nthreads,
...
std::tuple<Tensor, Tensor> ps_roi_align_forward_kernel(
  ...
  THO_DISPATCH_V2(
      ...
      AT_WRAP([&]() {
        launch_ps_roi_align_forward_kernel_impl<scalar_t>(
            grid,
            block,
            stream,
            output_size,
...
Tensor ps_roi_align_backward_kernel(
  ...
  THO_DISPATCH_V2(
      ...
      AT_WRAP([&]() {
        launch_ps_roi_align_backward_kernel_impl<scalar_t>(
            grid,
            block,
            stream,
            grad.numel(),
