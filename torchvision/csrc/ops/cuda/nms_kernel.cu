...
template <typename T>
__global__ void nms_rotated_kernel_impl(
    int n_boxes,
    double iou_threshold,
    const T* dev_boxes,
    unsigned long long* dev_mask) {
  ...
  // Compared to nms_kernel_impl, where each box is represented with 4 values
  ...
  if (threadIdx.x < row_size) {
    ...
    for (i = start; i < col_size; i++) {
      // Instead of devIoU used by nms_kernel_impl, here
...
at::Tensor nms_rotated_kernel(
  ...
  TORCH_CHECK(
      ...
      "dets and scores must have the same dtype");
  ...
  AT_DISPATCH_FLOATING_TYPES(
      dets_sorted.scalar_type(), "nms_rotated_kernel", [&] {
        nms_rotated_kernel_impl<scalar_t><<<blocks, threads, 0, stream>>>(
            dets_num,
            iou_threshold,
            dets_sorted.data_ptr<scalar_t>(),
            (unsigned long long*)mask.data_ptr<int64_t>());
